...
?>
  ...
  <div class="panel-heading">我的收入明细</div>
    
<div class="panel-body">
    您总共收入: <strong><?php
    $rs = $db->get_one("select sum(h_price) as sumP from `h_log_point2` where h_userName = '{$memberLogged_userName}' and h_price > 0 and (h_type='动态奖励' or h_type='分红' or (h_type='管理员操作' and h_about='补发结算'))");
	if(strlen($rs['sumP']) <= 0){
		$rs['sumP'] = 0;
	}
