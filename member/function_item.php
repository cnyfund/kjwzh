...
function settle_farm_day($userName){
	...
	while($rs = $db->fetch_array($query)){
		$addtime = strtotime($rs['h_addTime']);
		if($addtime>=strtotime(date('Y-m-d')) && !$rs['h_lastSettleTime']){
			continue;
		}
...
function settle_farm_ztj($buyUserName,$bonusAll,$floorIndex = 2){
	...
	//奖金
	if($floorIndex<=4){
		$bonus = floatval($bonusAll) * floatval($webInfo['h_point'. $floorIndex.'ComBuy'])/100;
	}else{
		$bonus = floatval($bonusAll) * floatval($webInfo['h_point5ComBuy'])/100;
	}
	...
	if($rs){
		...
		if(strlen($rs['h_parentUserName']) > 0){
			...
			//记录
				...
				$sql .= "h_price = '" . $bonus . "', ";
				$sql .= "h_type = '奖励', ";
				$sql .= "h_about = '第" . $floor . "代会员" . $buyUserName . "下级下单获得直推奖收入', ";
				$sql .= "h_addTime = '" . date('Y-m-d H:i:s') . "', ";
				$sql .= "h_actIP = '" . getUserIP() . "' ";
				$db->query($sql);
				//echo $sql . '<br />';
			...
			//下一轮
			settle_farm_ztj($rs['h_parentUserName'],$bonusAll,$floorIndex + 1);
		}
	}
