...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="tableborder">
  <tr>
    <td height="25" colspan="11" align="center" class="tdtitle">会员物品列表</td>
  </tr>
  <tr align="center"> 
    <td height="23" class="tdtitle-title">会员帐号</td>
    <td class="tdtitle-title">物品</td>
...
?>
  <tr align="center" class="tdbottom" onMouseOver="javascript:this.className='tdbottomover';" onMouseOut="javascript:this.className='tdbottom';"> 
    ...
    <td height="25"><?php echo $val['h_endTime']; ?></td>
    <td height="25"><?php echo $val['h_lastSettleTime']; ?></td>
    <td height="25"><?php echo $val['h_point2Day'] , '×' , $val['h_num'] , '=' , ($val['h_point2Day'] * $val['h_num']) , '元'; ?></td>
    ...
    <?php if(!$val['h_isEnd']){
			...
			if($mustSettleDay >0){
		?>
		<a style="cursor:pointer;" onClick="javascript:hintandturn('确认要结算该会员的农场分红数据吗？','?clause=settle&username=<?php echo urlencode($val['h_userName']); ?>',true);">结算</a>
		<?php
			}
	} ?>
	   <?php if($val['h_isEnd']){  ?>
