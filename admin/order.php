...
function main()
{
	...
	global $page;
	$list_num = 15;
	$total_count = $db->counter("`h_member_shop_order`", "1 = 1 {$where}", 'id');$page = (int)$page;$rowset = new Pager($total_count,$list_num,$page);$from_record = $rowset->_offset();$page_list = $rowset->link('?page=');
...
<?php if($rs['h_isReturn']){echo '<span style="color:#ff0000">注意：该订单已经打还元给会员！</span>';} ?>
</td>
		<td>
<?php
