...
function saveinfo()
{
	...
	*/
	...
	if(strlen($pwd) < 6 || strlen($pwd) > 32){
		HintAndBack("登录密码6-32位任意字符，请检查！",1);
	}
	if(strlen($pwdII) < 6 || strlen($pwdII) > 32){
		HintAndBack("安全密码6-32位任意字符，请检查！",1);
	}
...
function main()
{
	...
	global $page;
	$list_num = 20;
	$total_count = $db->counter("`h_member`", "1 = 1 {$where}", 'id');$page = (int)$page;$rowset = new Pager($total_count,$list_num,$page);
	$from_record = $rowset->_offset();
	$page_list = $rowset->link("?levelIndex={$levelIndex}&stype={$stype}&keyword={$keyword}&page=");
	$query = "select *,(select count(id) from `h_member` where h_parentUserName = a.h_userName and h_isPass = 1) as comMembers from (select * from `h_member` where 1 = 1 {$where} order by h_regTime desc,id desc LIMIT $from_record, $list_num) a";
...
function unlockinfo()
{
	...
	$query = "update `h_member` set h_isPass = 1 where id = $id";
	$db->query($query);
...
function lockinfo()
{
	...
	$query = "update `h_member` set h_isPass = 0 where id = $id";
	$db->query($query);
