...
function saveeditinfo()
{
	...
	$query = "update `h_point2_shop` SET 
			  h_title = '$h_title',
			  h_pic = '$h_pic',
...
function editinfo()
{
	...
	$rs = $db->get_one("SELECT * FROM `h_point2_shop` where id = $id");
	if(!$rs)
	{
...
function saveinfo()
{
	...
	$query = "insert into `h_point2_shop` SET 
			  h_title = '$h_title',
			  h_pic = '$h_pic',
			  h_minComMembers = '$h_minComMembers',
			  h_money = '$h_money',
...
function main()
{
	...
	global $page;
	$list_num = 15;
	$total_count = $db->counter("`h_point2_shop`", "1 = 1 {$where}", 'id');$page = (int)$page;$rowset = new Pager($total_count,$list_num,$page);$from_record = $rowset->_offset();$page_list = $rowset->link('?page=');
	$query = "select * from (select * from `h_point2_shop` where 1 = 1 {$where} order by h_addTime desc,id desc LIMIT $from_record, $list_num) a";
	$result = $db->query($query);
	//$query = "Select * from `h_point2_shop` order by h_addTime desc";
	//$result = $db->query($query);
	$rs_list = array();
...
function unvipinfo()
{
	...
	$query = "update `h_point2_shop` set h_isVIP = 1 where id = $id";
	$db->query($query);
...
function vipinfo()
{
	...
	$query = "update `h_point2_shop` set h_isVIP = 0 where id = $id";
	$db->query($query);
...
function unlockinfo()
{
	...
	$query = "update `h_point2_shop` set h_isPass = 1 where id = $id";
	$db->query($query);
...
function lockinfo()
{
	...
	$query = "update `h_point2_shop` set h_isPass = 0 where id = $id";
	$db->query($query);
...
function delinfo()
{
	...
	$query = "delete from `h_point2_shop` where id = $id";
	$db->query($query);
