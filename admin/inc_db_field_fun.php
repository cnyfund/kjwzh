...
function admin_article_main()
{
	...
	$total_count = $db->counter("`" . $admin_article_page_config['config']['dbTable'] . "`", $whereLM, 'id');$page = (int)$page;$rowset = new Pager($total_count,$list_num,$page);$from_record = $rowset->_offset();$page_list = $rowset->link('?' . $pageParms . '&page=');
	$query = "select " . $sql_select_field . " from `" . $admin_article_page_config['config']['dbTable'] . "` a where {$whereLM} order by {$orderBy} LIMIT $from_record, $list_num";
	//echo $query;
