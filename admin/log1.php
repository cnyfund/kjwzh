...
function main()
{
	...
	global $page;
	$list_num = 15;
	$total_count = $db->counter("`t_log_login_member`", "1 = 1 {$where}", 'id');$page = (int)$page;$rowset = new Pager($total_count,$list_num,$page);$from_record = $rowset->_offset();$page_list = $rowset->link('?page=');
	$query = "select * from `t_log_login_member` where 1 = 1 {$where} order by h_addTime desc,id desc LIMIT $from_record, $list_num";
...
function delinfo()
{
	...
	$query = "delete from `t_log_login_member` where id = $id";
	$db->query($query);
