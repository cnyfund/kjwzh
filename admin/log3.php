...
function main()
{
	...
	if(strlen($stype) > 0){
		switch($stype){
			case '元转账(入)':
				$where .= " and h_type = '元转账' and h_price > 0";
				break;
			case '元转账(出)':
				$where .= " and h_type = '元转账' and h_price < 0";
				break;
	...
	global $page;
	$list_num = 15;
	$total_count = $db->counter("`h_log_point2`", "1 = 1 {$where}", 'id');$page = (int)$page;$rowset = new Pager($total_count,$list_num,$page);$from_record = $rowset->_offset();$page_list = $rowset->link("?stype={$stype}&keyword={$keyword}&page=");
...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="tableborder">
  <tr>
    <td height="25" colspan="11" align="center" class="tdtitle">元流水明细</td>
  </tr>
  <tr align="center"> 
    <td class="tdtitle-title">类型</td>
    <td height="23" class="tdtitle-title">会员帐号</td>
    <td class="tdtitle-title">元</td>
...
<option value="元拍卖交易完成" <?php if($stype == '元拍卖交易完成'){echo "selected";}?>>元拍卖交易完成</option>
<option value="取消元购买" <?php if($stype == '取消元购买'){echo "selected";}?>>取消元购买</option>
<option value="元拍卖" <?php if($stype == '元拍卖'){echo "selected";}?>>元拍卖</option>
<option value="购买物品" <?php if($stype == '购买物品'){echo "selected";}?>>购买物品</option>
<option value="分红" <?php if($stype == '分红'){echo "selected";}?>>分红</option>
<option value="分红分红" <?php if($stype == '分红分红'){echo "selected";}?>>分红分红</option>
...
<option value="元转账" <?php if($stype == '元转账'){echo "selected";}?>>元转账</option>
<option value="元转账(入)" <?php if($stype == '元转账(入)'){echo "selected";}?>>元转账(入)</option>
<option value="元转账(出)" <?php if($stype == '元转账(出)'){echo "selected";}?>>元转账(出)</option>
